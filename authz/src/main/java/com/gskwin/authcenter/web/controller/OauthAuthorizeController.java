...
import com.gskwin.authcenter.oauth.authorize.CodeAuthorizeHandler;
import com.gskwin.authcenter.oauth.authorize.TokenAuthorizeHandler;
...
public class OauthAuthorizeController {
	...
	private void unsupportResponseType(OAuthAuthxRequest oauthRequest, HttpServletResponse response) throws OAuthSystemException {
		...
		OAuthResponse oAuthResponse = OAuthResponse.errorResponse(HttpServletResponse.SC_BAD_REQUEST).setError(OAuthError.CodeResponse.UNSUPPORTED_RESPONSE_TYPE).setErrorDescription("Unsupport response_type '" + responseType + "'")
				.buildJSONMessage();
		WebUtils.writeOAuthJsonResponse(response, oAuthResponse);
	}
